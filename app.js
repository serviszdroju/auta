...
function vehicleCard(v){const main=v.status==='free'?`<button class="main-action" onclick="openTake('${v.id}')">Převzít</button>`:v.status==='used'?`<button class="main-action" onclick="openReturn('${v.id}')">Vrátit</button>`:`<button class="main-action" onclick="openReservation('${v.id}')">Rezervovat</button>`;return `<article class="vehicle-card"><div class="vehicle-top"><span class="status-dot status-${v.status}"></span><div class="vehicle-title"><h3>${escapeHtml(v.name)}</h3><div class="plate">${escapeHtml(v.plate)}</div></div><span class="badge">${escapeHtml(v.type)}</span></div><div class="vehicle-state"><strong>${statusLabel(v.status)}</strong><span class="muted">${v.driver?escapeHtml(v.driver):escapeHtml(v.note||'Vozidlo je k dispozici')}</span></div><div class="info-grid"><div>Stav km<strong>${Number(v.mileage).toLocaleString('cs-CZ')} km</strong></div><div>STK do<strong>${fmtDate(v.stk)}</strong></div><div>VIN<strong>${escapeHtml(v.vin||'—')}</strong></div><div>Pojištění do<strong>${fmtDate(v.insuranceTo)}</strong></div><div>Palivo<strong>${escapeHtml(v.fuel||'—')}</strong></div><div>Vrácení<strong>${v.returnAt?fmtDateTime(v.returnAt):'—'}</strong></div></div><div class="card-actions">${main}<button onclick="openVehicle('${v.id}')">Detail</button><button class="qr-button" onclick="openQr('${v.id}')">QR kód vozidla</button></div></article>`}
...
window.openVehicle=id=>{const v=id?vehicle(id):null;editingId=id||null;openModal('vehicle',v?'Upravit vozidlo':'Přidat vozidlo','Vozový park',`<div class="form-grid">${field('SPZ','fPlate','text',v?.plate||'','required')}${field('Značka a model','fName','text',v?.name||'','required')}
...
$$('[data-close]').forEach(b=>b.onclick=()=>$('#modal').close());$$('[data-open="vehicle"]').forEach(b=>b.onclick=()=>openVehicle());$$('[data-open="reservation"]').forEach(b=>b.onclick=()=>openReservation());$$('[data-open="issue"]').forEach(b=>b.onclick=openIssue);
